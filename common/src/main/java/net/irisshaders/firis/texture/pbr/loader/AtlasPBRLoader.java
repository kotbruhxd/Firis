package net.irisshaders.firis.texture.pbr.loader;

import com.mojang.blaze3d.platform.NativeImage;
import net.irisshaders.firis.Firis;
import net.irisshaders.firis.mixin.texture.AnimationMetadataSectionAccessor;
import net.irisshaders.firis.mixin.texture.TextureAtlasAccessor;
import net.irisshaders.firis.texture.format.TextureFormat;
import net.irisshaders.firis.texture.format.TextureFormatLoader;
import net.irisshaders.firis.texture.mipmap.ChannelMipmapGenerator;
import net.irisshaders.firis.texture.mipmap.CustomMipmapGenerator;
import net.irisshaders.firis.texture.mipmap.LinearBlendFunction;
import net.irisshaders.firis.texture.pbr.PBRAtlasTexture;
...
import net.irisshaders.firis.texture.util.ImageManipulationUtil;
...
public class AtlasPBRLoader implements PBRTextureLoader<TextureAtlas> {
	...
	protected PBRTextureAtlasSprite createPBRSprite(TextureAtlasSprite sprite, ResourceManager resourceManager, TextureAtlas atlas, int atlasWidth, int atlasHeight, int mipLevel, PBRType pbrType) {
		...
		try {
			...
			Firis.logger.error("Unable to parse metadata from {}", pbrImageLocation, e);
			return null;
		}
		...
		try (InputStream stream = resource.open()) {
			...
			Firis.logger.error("Using missing texture, unable to load {}", pbrImageLocation, e);
			return null;
		}
		...
		if (!Mth.isMultipleOf(imageWidth, frameWidth) || !Mth.isMultipleOf(imageHeight, frameHeight)) {
			Firis.logger.error("Image {} size {},{} is not multiple of frame size {},{}", pbrImageLocation, imageWidth, imageHeight, frameWidth, frameHeight);
			nativeImage.close();
			return null;
		}
		...
		if (frameWidth != targetFrameWidth || frameHeight != targetFrameHeight) {
			try {
				...
				Firis.logger.error("Something bad happened trying to load PBR texture " + spriteName.getPath() + pbrType.getSuffix() + "!", e);
				throw e;
			}
		}
