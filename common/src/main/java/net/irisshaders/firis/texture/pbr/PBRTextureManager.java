...
import net.irisshaders.firis.texture.pbr.loader.PBRTextureLoader;
import net.irisshaders.firis.texture.pbr.loader.PBRTextureLoader.PBRTextureConsumer;
import net.irisshaders.firis.texture.pbr.loader.PBRTextureLoaderRegistry;
...
public class PBRTextureManager {
	...
	private static void dumpTexture(Dumpable dumpable, ResourceLocation id, Path path) {
		try {
			...
			Firis.logger.error("Failed to dump texture {}", id, e);
		}
	}
		...
		if (texture != null) {
			...
			if (loader != null) {
				...
				try {
					...
					Firis.logger.debug("Failed to load PBR textures for texture " + id, e);
				} finally {
