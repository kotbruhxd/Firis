package net.irisshaders.firis.api.v0;

import net.irisshaders.firis.apiimpl.FirisApiV0Impl;

import java.nio.ByteBuffer;
import java.util.function.IntFunction;

/**
 * The entry point to the Firis API, major version 0. This is currently the latest
 * version of the API.
 * <p>
 * To access the API, use {@link #getInstance()}.
 */
public interface FirisApi {
	/**
	 * @since API v0.0
	 */
	static FirisApi getInstance() {
		return FirisApiV0Impl.INSTANCE;
	}
	 ...
	 * the currently installed Firis version.
	 *
	 ...
	 * very custom culling that doesn't work when the Firis shadow
	 * pass is active.
	 *
	 * <p>Naturally, this function can only return true if
	 * {@link #isShaderPackInUse()} returns true.
	 *
	 * @return Whether Firis is currently rendering the shadow pass.
	 * @since API v0.0
	 */
	boolean isRenderingShadowPass();

	/**
	 * Opens the main Firis GUI screen. It's up to Firis to decide
	 ...
	 *               {@code Screen} class.
	 * @return A {@code Screen} class for the main Firis GUI screen.
	 * @since API v0.0
	 */
	...
	/**
	 * Gets a config object that can edit the Firis configuration.
	 *
	 * @since API v0.0
	 */
	FirisApiConfig getConfig();
	 ...
	 * @since API 0.1
	 */
	FirisTextVertexSink createTextVertexSink(int maxQuadCount, IntFunction<ByteBuffer> bufferProvider);
