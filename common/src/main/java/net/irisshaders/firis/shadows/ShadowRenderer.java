package net.irisshaders.firis.shadows;
...
import net.irisshaders.firis.shaderpack.properties.ShadowCullState;
import net.irisshaders.firis.shadows.frustum.BoxCuller;
import net.irisshaders.firis.shadows.frustum.CullEverythingFrustum;
import net.irisshaders.firis.shadows.frustum.FrustumHolder;
import net.irisshaders.firis.shadows.frustum.advanced.AdvancedShadowCullingFrustum;
import net.irisshaders.firis.shadows.frustum.advanced.ReversedAdvancedShadowCullingFrustum;
import net.irisshaders.firis.shadows.frustum.fallback.BoxCullingFrustum;
import net.irisshaders.firis.shadows.frustum.fallback.NonCullingFrustum;
import net.irisshaders.firis.uniforms.CameraUniforms;
import net.irisshaders.firis.uniforms.CapturedRenderingState;
import net.irisshaders.firis.uniforms.CelestialUniforms;
...
public class ShadowRenderer {
	...
	private void configureDepthSampler(int glTextureId, PackShadowDirectives.DepthSamplingSettings settings) {
		if (settings.getHardwareFiltering() && !separateHardwareSamplers) {
			...
			FirisRenderSystem.texParameteri(glTextureId, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_COMPARE_MODE, GL30C.GL_COMPARE_REF_TO_TEXTURE);
		}
		...
		FirisRenderSystem.texParameteriv(glTextureId, GL20C.GL_TEXTURE_2D, ARBTextureSwizzle.GL_TEXTURE_SWIZZLE_RGBA,
	...
	private void configureSampler(int glTextureId, PackShadowDirectives.SamplingSettings settings) {
		...
		if (!settings.getNearest()) {
			...
			FirisRenderSystem.texParameteri(glTextureId, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MIN_FILTER, GL20C.GL_LINEAR);
			FirisRenderSystem.texParameteri(glTextureId, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MAG_FILTER, GL20C.GL_LINEAR);
		} else {
			FirisRenderSystem.texParameteri(glTextureId, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MIN_FILTER, GL20C.GL_NEAREST);
			FirisRenderSystem.texParameteri(glTextureId, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MAG_FILTER, GL20C.GL_NEAREST);
		}
	}
	...
	private void setupMipmappingForTexture(int texture, int filteringMode) {
		...
		FirisRenderSystem.texParameteri(texture, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MIN_FILTER, filteringMode);
	}
	...
	private FrustumHolder createShadowFrustum(float renderMultiplier, FrustumHolder holder) {
		...
		if ((packCullingState == ShadowCullState.DEFAULT && packHasVoxelization) || packCullingState == ShadowCullState.DISTANCE) {
			...
			if (renderMultiplier < 0) {
				distance = FirisVideoSettings.shadowDistance * 16;
				setter = "(set by user)";
			}
	...
	public void renderShadows(LevelRendererAccessor levelRenderer, Camera playerCamera) {
		if (FirisVideoSettings.getOverriddenShadowDistance(FirisVideoSettings.shadowDistance) == 0) {
			return;
		}
		...
		if (renderDistanceMultiplier < 0) {
			renderDistance = FirisVideoSettings.shadowDistance;
		}
		...
		FirisRenderSystem.setShadowProjection(shadowProjection);
		...
		FirisRenderSystem.restorePlayerProjection();
	...
	public void addDebugText(List<String> messages) {
		if (FirisVideoSettings.getOverriddenShadowDistance(FirisVideoSettings.shadowDistance) == 0) {
			messages.add("[" + Firis.MODNAME + "] Shadow Maps: off, shadow distance 0");
			return;
		}

		if (Firis.getFirisConfig().areDebugOptionsEnabled()) {
			messages.add("[" + Firis.MODNAME + "] Shadow Maps: " + debugStringOverall);
			messages.add("[" + Firis.MODNAME + "] Shadow Distance Terrain: " + terrainFrustumHolder.getDistanceInfo() + " Entity: " + entityFrustumHolder.getDistanceInfo());
			messages.add("[" + Firis.MODNAME + "] Shadow Culling Terrain: " + terrainFrustumHolder.getCullingInfo() + " Entity: " + entityFrustumHolder.getCullingInfo());
			messages.add("[" + Firis.MODNAME + "] Shadow Terrain: " + debugStringTerrain
				+ (shouldRenderTerrain ? "" : " (no terrain) ") + (shouldRenderTranslucent ? "" : "(no translucent)"));
			messages.add("[" + Firis.MODNAME + "] Shadow Entities: " + getEntitiesDebugString());
			messages.add("[" + Firis.MODNAME + "] Shadow Block Entities: " + getBlockEntitiesDebugString());

			if (buffers instanceof DrawCallTrackingRenderBuffers drawCallTracker && (shouldRenderEntities || shouldRenderPlayer)) {
				messages.add("[" + Firis.MODNAME + "] Shadow Entity Batching: " + BatchingDebugMessageHelper.getDebugMessage(drawCallTracker));
			}
		} else {
			messages.add("[" + Firis.MODNAME + "] Shadow info: " + debugStringTerrain);
			messages.add("[" + Firis.MODNAME + "] E: " + renderedShadowEntities);
			messages.add("[" + Firis.MODNAME + "] BE: " + renderedShadowBlockEntities);
		}
	}
