...
public class ShadowRenderTargets {
	...
	public ShadowRenderTargets(WorldRenderingPipeline pipeline, int resolution, PackShadowDirectives shadowDirectives) {
		...
		this.size = pipeline.hasFeature(FeatureFlags.HIGHER_SHADOWCOLOR) ? PackShadowDirectives.MAX_SHADOW_COLOR_BUFFERS_FIRIS : PackShadowDirectives.MAX_SHADOW_COLOR_BUFFERS_OF;
	...
	public void copyPreTranslucentDepth() {
		if (translucentDepthDirty) {
			...
			FirisRenderSystem.blitFramebuffer(depthSourceFb.getId(), noTranslucentsDestFb.getId(), 0, 0, resolution, resolution,
	...
	public GlFramebuffer createColorFramebuffer(ImmutableSet<Integer> stageWritesToMain, int[] drawBuffers) {
		...
		for (int i = 0; i < drawBuffers.length; i++) {
			...
			if (drawBuffers[i] >= getRenderTargetCount()) {
				...
				// Firis.logger.warn("Invalid framebuffer was attempted to be created! Forcing a framebuffer with DRAWBUFFERS 01 for shadow.");
