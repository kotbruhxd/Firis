...
public class ShadowCompositeRenderer {
	...
	private static void setupMipmapping(net.irisshaders.firis.targets.RenderTarget target, boolean readFromAlt) {
		...
		FirisRenderSystem.texParameteri(texture, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MIN_FILTER, target.getInternalFormat().getPixelFormat().isInteger() ? GL20C.GL_NEAREST_MIPMAP_NEAREST : GL20C.GL_LINEAR_MIPMAP_LINEAR);
	}
	...
	private Program createProgram(ProgramSource source, ImmutableSet<Integer> flipped, ImmutableSet<Integer> flippedAtLeastOnceSnapshot,
		...
		FirisSamplers.addCustomTextures(customTextureSamplerInterceptor, irisCustomTextures);

		FirisSamplers.addShadowSamplers(customTextureSamplerInterceptor, targets, flipped, pipeline.hasFeature(FeatureFlags.SEPARATE_HARDWARE_SAMPLERS));
		FirisImages.addShadowColorImages(builder, targets, flipped);
		FirisImages.addCustomImages(builder, irisCustomImages);
		FirisSamplers.addCustomImages(builder, irisCustomImages);
