...
public class ShaderPackSelectionList extends FirisObjectSelectionList<ShaderPackSelectionList.BaseEntry> {
	...
	public ShaderPackSelectionList(ShaderPackScreen screen, Minecraft client, int width, int height, int top, int bottom, int left, int right) {
		...
		this.screen = screen;
		this.topButtonRow = new TopButtonRowEntry(this, Firis.getFirisConfig().areShadersEnabled());
		try {
			watcher1 = FileSystems.getDefault().newWatchService();
			key1 = Firis.getShaderpacksDirectory().register(watcher1,
				StandardWatchEventKinds.ENTRY_CREATE,
				StandardWatchEventKinds.ENTRY_MODIFY,
				StandardWatchEventKinds.ENTRY_DELETE);
			keyValid = true;
		} catch (IOException e) {
			Firis.logger.error("Couldn't register file watcher!", e);
			watcher1 = null;
			key1 = null;
			keyValid = false;
		}
	...
	public void refresh() {
		...
		try {
			names = Firis.getShaderpacksDirectoryManager().enumerate();
		} catch (Throwable e) {
			Firis.logger.error("Error reading files while constructing selection UI", e);
			...
			// - https://github.com/FirisShaders/Firis/issues/785
	...
	public void addPackEntry(int index, String name) {
		...
		Firis.getFirisConfig().getShaderPackName().ifPresent(currentPackName -> {
	...
	public static class TopButtonRowEntry extends BaseEntry {
		...
		public static class EnableShadersButtonElement extends FirisElementRow.TextButtonElement {
			private int centerX;

			public EnableShadersButtonElement(Component text, Function<FirisElementRow.TextButtonElement, Boolean> onClick) {
				super(text, onClick);
			}
