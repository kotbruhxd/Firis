...
import net.irisshaders.firis.vertices.views.QuadView;
...
public class FirisTextVertexSinkImpl implements FirisTextVertexSink {
	private static final int STRIDE = FirisVertexFormats.GLYPH.getVertexSize();
	...
	static VertexFormat format = FirisVertexFormats.GLYPH;
	...
	public FirisTextVertexSinkImpl(int maxQuadCount, IntFunction<ByteBuffer> buffer) {
	...
	private void vertex(float x, float y, float z, int color, float u, float v, int light) {
		...
		if (vertexCount == 4) {
			...
			uSum *= 0.25f;
			vSum *= 0.25f;
			quad.setup(elementOffset, FirisVertexFormats.GLYPH.getVertexSize());
