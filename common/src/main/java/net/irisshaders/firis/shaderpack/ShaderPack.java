...
import net.irisshaders.firis.gui.FeatureMissingErrorScreen;
...
import net.irisshaders.firis.shaderpack.include.IncludeGraph;
import net.irisshaders.firis.shaderpack.include.IncludeProcessor;
import net.irisshaders.firis.shaderpack.include.ShaderPackSourceNames;
...
import net.irisshaders.firis.shaderpack.option.menu.OptionMenuContainer;
...
import net.irisshaders.firis.shaderpack.parsing.BooleanParser;
import net.irisshaders.firis.shaderpack.preprocessor.JcppProcessor;
...
import net.irisshaders.firis.shaderpack.programs.ProgramSetInterface;
...
public class ShaderPack {
	...
	public ShaderPack(Path root, Map<String, String> changedConfigs, ImmutableList<StringPair> environmentDefines) throws IOException, IllegalStateException {
		...
		envDefines1.addAll(FirisDefines.createIrisReplacements());
		...
		if (!graph.getFailures().isEmpty()) {
			graph.getFailures().forEach((path, error) -> {
				Firis.logger.error("{}", error.toString());
			});
		...
		for (FeatureFlags flag : FeatureFlags.values()) {
			if (flag.isUsable()) {
				finalEnvironmentDefines.add(new StringPair("FIRIS_FEATURE_" + flag.name(), ""));
			}
		}
		...
		if (!activeFeatures.contains(FeatureFlags.SSBO) && !shaderProperties.getBufferObjects().isEmpty()) {
			throw new IllegalStateException("An SSBO is being used, but the feature flag for SSBO's hasn't been set! Please set either a requirement or check for the SSBO feature using \"firis.features.required/optional = ssbo\".");
		}

		if (!activeFeatures.contains(FeatureFlags.CUSTOM_IMAGES) && !shaderProperties.getIrisCustomImages().isEmpty()) {
			throw new IllegalStateException("Custom images are being used, but the feature flag for custom images hasn't been set! Please set either a requirement or check for custom images' feature flag using \"firis.features.required/optional = CUSTOM_IMAGES\".");
		}
		...
		if (!invalidFeatureFlags.isEmpty()) {
			if (Minecraft.getInstance().screen instanceof ShaderPackScreen) {
				MutableComponent component = Component.translatable("firis.unsupported.pack.description", FeatureFlags.getInvalidStatus(invalidFlagList), invalidFeatureFlags.stream()
					.collect(Collectors.joining(", ", ": ", ".")));
				if (SystemUtils.IS_OS_MAC) {
					component = component.append(Component.translatable("firis.unsupported.pack.macos"));
				}
				Minecraft.getInstance().setScreen(new FeatureMissingErrorScreen(Minecraft.getInstance().screen, Component.translatable("firis.unsupported.pack"), component));
			}
			FirisApi.getInstance().getConfig().setShadersEnabledAndApply(false);
		}
		...
		if (!optionalFeatureFlags.isEmpty()) {
			optionalFeatureFlags.forEach(flag -> newEnvDefines.add(new StringPair("FIRIS_FEATURE_" + flag, "")));
		}
		...
		Firis.logger.info(this.profileInfo);
		...
		customNoiseTexture = shaderProperties.getNoiseTexturePath().map(path -> {
			try {
				...
				Firis.logger.error("Unable to read the custom noise texture at " + path, e);
		...
		shaderProperties.getCustomTextures().forEach((textureStage, customTexturePropertiesMap) -> {
			...
			customTexturePropertiesMap.forEach((samplerName, path) -> {
				try {
					...
					Firis.logger.error("Unable to read the custom texture at " + path, e);
				}
			});
		...
		shaderProperties.getIrisCustomTextures().forEach((name, texture) -> {
			try {
				...
				Firis.logger.error("Unable to read the custom texture at " + texture.getName(), e);
			}
		});
	}
	...
	private static Optional<Properties> loadProperties(Path shaderPath, String name,
		...
		// See https://github.com/FirisShaders/Firis/issues/1327 and the relevant putIfAbsent calls in
		// BlockMaterialMapping
		Properties properties = new OrderBackedProperties();
		try {
			properties.load(propertiesReader);
		} catch (IOException e) {
			Firis.logger.error("Error loading " + name + " at " + shaderPath, e);
	...
	private static String readProperties(Path shaderPath, String name) {
		try {
			...
			Firis.logger.debug("An " + name + " file was not found in the current shaderpack");

			return null;
		} catch (IOException e) {
			Firis.logger.error("An IOException occurred reading " + name + " from the current shaderpack", e);
	...
	public CustomTextureData readTexture(Path root, TextureDefinition definition) throws IOException {
		...
		if (path.contains(":")) {
			...
			if (parts.length > 2) {
				Firis.logger.warn("Resource location " + path + " contained more than two parts?");
			}
			...
			if (Files.exists(mcMetaResolvedPath)) {
				try {
					...
					Firis.logger.error("Unable to read the custom texture mcmeta at " + mcMetaPath + ", ignoring: " + e);
				}
			}
		...
		overrides = this.overrides.computeIfAbsent(dimension, dim -> {
			if (dimensionMap.containsKey(dimension)) {
				...
				if (dimensionIds.contains(name)) {
					...
					Firis.logger.error("Attempted to load dimension folder " + name + " for dimension " + dimension + ", but it does not exist!");
