...
import net.irisshaders.firis.shaderpack.parsing.ComputeDirectiveParser;
import net.irisshaders.firis.shaderpack.parsing.ConstDirectiveParser;
import net.irisshaders.firis.shaderpack.parsing.DispatchingDirectiveHolder;
...
public class ProgramSet implements ProgramSetInterface {
	...
	public ProgramSet(AbsolutePackPath directory, Function<AbsolutePackPath, String> sourceProvider,
		...
		//
		// - https://github.com/FirisShaders/Firis/issues/483
		// - https://github.com/FirisShaders/Firis/issues/987
	...
	private static ProgramSource readProgramSource(AbsolutePackPath directory,
		...
		if (vertexSource == null && fragmentSource != null) {
			...
			Firis.logger.warn("Found a program (" + program + ") that has a fragment shader but no vertex shader? This is very legacy behavior and might not work right.");
			vertexSource = """
				#version 120
	...
	private void locateDirectives() {
		...
		packDirectives.getRenderTargetDirectives().getRenderTargetSettings().forEach((index, settings) ->
			Firis.logger.debug("Render target settings for colortex" + index + ": " + settings));
	}
