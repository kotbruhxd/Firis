...
import net.irisshaders.firis.shaderpack.materialmap.BlockEntry;
import net.irisshaders.firis.shaderpack.materialmap.BlockRenderType;
import net.irisshaders.firis.shaderpack.materialmap.Entry;
import net.irisshaders.firis.shaderpack.materialmap.LegacyIdMap;
import net.irisshaders.firis.shaderpack.materialmap.NamespacedId;
import net.irisshaders.firis.shaderpack.materialmap.TagEntry;
...
public class IdMap {
	...
	private static Optional<Properties> loadProperties(Path shaderPath, String name, ShaderPackOptions shaderPackOptions,
		...
		if (Firis.getFirisConfig().areDebugOptionsEnabled()) {
			...
			try (OutputStream os = Files.newOutputStream(FirisPlatformHelpers.getInstance().getGameDir().resolve("patched_shaders").resolve(name))) {
	...
	private static Object2IntMap<NamespacedId> parseIdMap(Properties properties, String keyPrefix, String fileName) {
		...
		properties.forEach((keyObject, valueObject) -> {
			...
			try {
				...
				Firis.logger.warn("Failed to parse line in " + fileName + ": invalid key " + key);
				return;
			}
			...
			for (String part : value.split("\\s+")) {
				if (part.contains("=")) {
					...
					Firis.logger.warn("Failed to parse an ResourceLocation in " + fileName + " for the key " + key + ": state properties are currently not supported: " + part);
					continue;
				}
		...
		properties.forEach((keyObject, valueObject) -> {
			...
			for (String part : value.split("\\s+")) {
				...
				try {
					...
					Firis.logger.warn("Unexpected error while parsing an entry from " + fileName + " for the key " + key + ":", e);
				}
			}
	...
	private static Map<NamespacedId, BlockRenderType> parseRenderTypeMap(Properties properties, String keyPrefix, String fileName) {
		...
		properties.forEach((keyObject, valueObject) -> {
			...
			if (renderType == null) {
				Firis.logger.warn("Failed to parse line in " + fileName + ": invalid block render type: " + key);
				return;
			}

			for (String part : value.split("\\s+")) {
				if (part.startsWith("%")) {
					Firis.logger.fatal("Cannot use a tag in the render type map: " + key + " = " + value);
					continue;
				}
				// Note: NamespacedId performs no validation on the content. That will need to be done by whatever is
				//       converting these things to ResourceLocations.
