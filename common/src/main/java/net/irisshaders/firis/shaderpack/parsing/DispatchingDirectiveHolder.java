package net.irisshaders.firis.shaderpack.parsing;

import it.unimi.dsi.fastutil.booleans.BooleanConsumer;
import it.unimi.dsi.fastutil.floats.FloatConsumer;
import net.irisshaders.firis.Firis;
import net.irisshaders.firis.FirisLogging;
...
public class DispatchingDirectiveHolder implements DirectiveHolder {
		...
		if (type == ConstDirectiveParser.Type.BOOL) {
			...
			if (consumer != null) {
				if ("true".equals(value)) {
					...
					Firis.logger.error("Failed to process " + directive + ": " + value + " is not a valid boolean value");
				}

				return;
			}

			if (FirisLogging.ENABLE_SPAM) {
				// Only logspam in dev
				Firis.logger.info("Found potential unhandled directive: " + directive);
			}
			...
			if (intConsumer != null) {
				try {
					...
					Firis.logger.error("Failed to process " + directive, e);
				}
			...
			if (consumer != null) {
				if (!value.startsWith("vec2")) {
					Firis.logger.error("Failed to process " + directive + ": value was not a valid vec2 constructor");
				}
				...
				if (parts.length != 2) {
					Firis.logger.error("Failed to process " + directive +
			...
			if (consumer != null) {
				if (!value.startsWith("ivec3")) {
					Firis.logger.error("Failed to process " + directive + ": value was not a valid ivec3 constructor");
				}
			...
			if (consumer != null) {
				if (!value.startsWith("vec4")) {
					Firis.logger.error("Failed to process " + directive + ": value was not a valid vec4 constructor");
				}
	...
	private void typeCheckHelper(String expected, Map<String, ?> candidates, ConstDirectiveParser.ConstDirective directive) {
		if (candidates.containsKey(directive.getKey())) {
			Firis.logger.warn("Ignoring " + directive + " because it is of the wrong type, a type of " + expected + " is expected.");
		}
	}
	...
	public void acceptUniformDirective(String name, Runnable onDetected) {
		// TODO
		if (FirisLogging.ENABLE_SPAM) {
			Firis.logger.warn("Not looking for a uniform directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
	...
	public void acceptCommentStringDirective(String name, Consumer<String> consumer) {
		// TODO
		if (FirisLogging.ENABLE_SPAM) {
			Firis.logger.warn("Not looking for a comment string directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
	...
	public void acceptCommentIntDirective(String name, IntConsumer consumer) {
		// TODO
		if (FirisLogging.ENABLE_SPAM) {
			Firis.logger.warn("Not looking for a comment int directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
	...
	public void acceptCommentFloatDirective(String name, FloatConsumer consumer) {
		// TODO
		if (FirisLogging.ENABLE_SPAM) {
			Firis.logger.warn("Not looking for a comment float directive with the name " + name + " since this type of directive is not currently supported.");
		}
	}
