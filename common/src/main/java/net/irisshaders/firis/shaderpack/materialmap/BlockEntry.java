			...
			if (splitStates.length == 1) {
				...
				for (int index = statesStart; index < splitStates.length; index++) {
					...
					if (propertyParts.length != 2) {
						Firis.logger.warn("Warning: the block ID map entry \"" + entry + "\" could not be fully parsed:");
						Firis.logger.warn("- Block state property filters must be of the form \"key=value\", but "
