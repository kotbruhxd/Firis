package net.irisshaders.firis.shaderpack.preprocessor;
...
public class PropertiesPreprocessor {
	...
	public static String preprocessSource(String source, ShaderPackOptions shaderPackOptions, Iterable<StringPair> environmentDefines) {
		if (source.contains(PropertyCollectingListener.PROPERTY_MARKER) || source.contains("FIRIS_PASSTHROUGHBACKSLASH")) {
			throw new RuntimeException("Some shader author is trying to exploit internal Firis implementation details, stop!");
		}
		...
		try (Preprocessor pp = new Preprocessor()) {
			...
			stringValues.forEach((name, value) -> {
				try {
					...
					Firis.logger.fatal("Failed to preprocess property file!", e);
				}
			});
	...
	private static String process(Preprocessor preprocessor, String source) {
		...
		source = source.replace("\\", "FIRIS_PASSTHROUGHBACKSLASH");
		...
		try {
			...
			Firis.logger.error("Properties pre-processing failed", e);
		}

		source = builder.toString();

		return (listener.collectLines() + source).replace("FIRIS_PASSTHROUGHBACKSLASH", "\\");
	}
