...
import net.irisshaders.firis.shaderpack.parsing.DirectiveHolder;
...
public class PackShadowDirectives {
	...
	public static final int MAX_SHADOW_COLOR_BUFFERS_FIRIS = 8;
	...
	public PackShadowDirectives(ShaderProperties properties) {
		...
		//
		// However, Firis does still does cull shadows whenever it can - but, it does so by analyzing
		...
		for (int i = 0; i < PackShadowDirectives.MAX_SHADOW_COLOR_BUFFERS_FIRIS; i++) {
			...
			directives.acceptConstStringDirective(bufferName + "Format", format -> {
				...
				if (internalFormat.isPresent()) {
					...
					Firis.logger.warn("Unrecognized internal texture format " + format + " specified for " + bufferName + "Format, ignoring.");
				}
			});
