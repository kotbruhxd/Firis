package net.irisshaders.firis.shaderpack.discovery;
...
public class ShaderpackDirectoryManager {
	...
	public void copyPackIntoDirectory(String name, Path source) throws IOException {
		Path target = Firis.getShaderpacksDirectory().resolve(name);
	...
	public List<String> enumerate() throws IOException {
		...
		boolean debug = Firis.getFirisConfig().areDebugOptionsEnabled();
		...
		try (Stream<Path> list = Files.list(root)) {
			return list.filter(Firis::isValidToShowPack)
				.sorted(comparator)
