package net.irisshaders.firis.features;
...
public enum FeatureFlags {
	...
	CUSTOM_IMAGES(() -> true, FirisRenderSystem::supportsImageLoadStore),
	PER_BUFFER_BLENDING(() -> true, FirisRenderSystem::supportsBufferBlending),
	COMPUTE_SHADERS(() -> true, FirisRenderSystem::supportsCompute),
	TESSELLATION_SHADERS(() -> true, FirisRenderSystem::supportsTesselation),
	ENTITY_TRANSLUCENT(() -> true, () -> true),
	REVERSED_CULLING(() -> true, () -> true),
	BLOCK_EMISSION_ATTRIBUTE(() -> true, () -> true),
	SSBO(() -> true, FirisRenderSystem::supportsSSBO),
	UNKNOWN(() -> false, () -> false);
	...
	public static String getInvalidStatus(List<FeatureFlags> invalidFeatureFlags) {
		...
		if (unsupportedIris) {
			if (unsupportedHardware) {
				return I18n.get("firis.unsupported.irisorpc");
			}

			return I18n.get("firis.unsupported.iris");
		} else if (unsupportedHardware) {
			return I18n.get("firis.unsupported.pc");
		} else {
			return null;
		}
	}
