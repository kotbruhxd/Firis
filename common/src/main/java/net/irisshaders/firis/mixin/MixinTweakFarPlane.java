...
public class MixinTweakFarPlane {
	...
	private float firis$tweakViewDistanceToMatchOptiFine(GameRenderer renderer) {
	...
	private void firis$tweakViewDistanceBasedOnFog(float f, long l, PoseStack poseStack, CallbackInfo ci) {
		if (!Firis.getCurrentPack().isPresent()) {
			// Don't mess with the far plane if no shaderpack is loaded
			return;
		}
