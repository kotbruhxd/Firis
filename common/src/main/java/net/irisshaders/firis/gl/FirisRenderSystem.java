 ...
 */
public class FirisRenderSystem {
	...
	public static void initRenderer() {
		if (GL.getCapabilities().OpenGL45) {
			...
			Firis.logger.info("OpenGL 4.5 detected, enabling DSA.");
		} else if (GL.getCapabilities().GL_ARB_direct_state_access) {
			dsaState = new DSAARB();
			Firis.logger.info("ARB_direct_state_access detected, enabling DSA.");
		} else {
			dsaState = new DSAUnsupported();
			Firis.logger.info("DSA support not detected.");
		}
	...
	public static void texImage1D(int texture, int target, int level, int internalformat, int width, int border, int format, int type, @Nullable ByteBuffer pixels) {
		RenderSystem.assertOnRenderThreadOrInit();
		FirisRenderSystem.bindTextureForSetup(target, texture);
	...
	/**
	 * Internal API for use when you don't know the target texture. Should use {@link FirisRenderSystem#texParameteriv(int, int, int, int[])} instead unless you know what you're doing!
	 */
