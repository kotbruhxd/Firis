package net.irisshaders.firis.gl.blending;
...
public class BlendModeStorage {
	...
	public static void overrideBufferBlend(int index, BlendMode override) {
		...
		if (override == null) {
			FirisRenderSystem.disableBufferBlend(index);
		} else {
			FirisRenderSystem.enableBufferBlend(index);
			FirisRenderSystem.blendFuncSeparatei(index, override.srcRgb(), override.dstRgb(), override.srcAlpha(), override.dstAlpha());
		}
