package net.irisshaders.firis.gl.shader;
...
public class StandardMacros {
	...
	public static ImmutableList<StringPair> createStandardEnvironmentDefines() {
		...
		define(standardDefines, "FIRIS_VERSION", getFormattedIrisVersion());
		...
		define(standardDefines, "IS_FIRIS");
		define(standardDefines, "FIRIS_TAG_SUPPORT", "2");


		if (FirisPlatformHelpers.getInstance().isModLoaded("distanthorizons") && DHCompat.hasRenderingEnabled()) {
			define(standardDefines, "DISTANT_HORIZONS");
		}

		if (FirisPlatformHelpers.getInstance().isModLoaded("continuity")) {
			define(standardDefines, "FIRIS_HAS_CONNECTED_TEXTURES");
		}
	...
	public static String getMcVersion() {
		String version = Firis.getReleaseTarget();
		if (version == null) {
		...
		if (formattedVersion == null) {
			Firis.logger.error("Could not parse game version \"" + version + "\"");
		} else {
			return formattedVersion;
		}
		String backupVersion = Firis.getBackupVersionNumber();
	...
	/**
	 * Gets the current Firis version String in a 5 digit format
	 *
	 * @return The Firis version string
	 *
	 */
	public static String getFormattedIrisVersion() {
		String rawVersion = Firis.getVersion();
		if (rawVersion == null) {
			throw new IllegalArgumentException("Could not get current Firis version!");
		}
		Matcher matcher = SEMVER_PATTERN.matcher(rawVersion);
		if (!matcher.matches()) {
			throw new IllegalArgumentException("Could not parse semantic Firis version from \"" + rawVersion + "\"");
		}
		...
		if (formattedSemver == null) {
			throw new IllegalArgumentException("Could not get a valid semantic version string for Firis version \"" + irisSemver + "\"");
		} else {
	...
	public static Set<String> getGlExtensions() {
		...
		//
		// https://github.com/FirisShaders/Firis/issues/971
	...
	/**
	 * Returns the list of Firis-exclusive uniforms supported in the current version of Firis.
	 *
	...
	public static List<String> getIrisDefines() {
		...
		// All Firis-exclusive uniforms should have a corresponding definition here. Example:
