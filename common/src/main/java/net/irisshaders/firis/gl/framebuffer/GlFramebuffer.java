package net.irisshaders.firis.gl.framebuffer;
...
public class GlFramebuffer extends GlResource {
	...
	public GlFramebuffer() {
		super(FirisRenderSystem.createFramebuffer());
	...
	public void addDepthAttachment(int texture) {
		...
		if (depthBufferFormat.isCombinedStencil()) {
			FirisRenderSystem.framebufferTexture2D(fb, GL30C.GL_FRAMEBUFFER, GL30C.GL_DEPTH_STENCIL_ATTACHMENT, GL30C.GL_TEXTURE_2D, texture, 0);
		} else {
			FirisRenderSystem.framebufferTexture2D(fb, GL30C.GL_FRAMEBUFFER, GL30C.GL_DEPTH_ATTACHMENT, GL30C.GL_TEXTURE_2D, texture, 0);
		}
	...
	public void addColorAttachment(int index, int texture) {
		...
		FirisRenderSystem.framebufferTexture2D(fb, GL30C.GL_FRAMEBUFFER, GL30C.GL_COLOR_ATTACHMENT0 + index, GL30C.GL_TEXTURE_2D, texture, 0);
		attachments.put(index, texture);
	}

	public void noDrawBuffers() {
		FirisRenderSystem.drawBuffers(getGlId(), new int[]{GL30C.GL_NONE});
	}
	...
	public void drawBuffers(int[] buffers) {
		...
		FirisRenderSystem.drawBuffers(getGlId(), glBuffers);
	}

	public void readBuffer(int buffer) {
		FirisRenderSystem.readBuffer(getGlId(), GL30C.GL_COLOR_ATTACHMENT0 + buffer);
	}
