...
import net.irisshaders.firis.gl.uniform.Uniform;
	...
	public static class Builder implements DynamicLocationalUniformHolder {
		...
		public OptionalInt location(String name, UniformType type) {
			...
			if ((!locations.containsKey(id) && !uniformNames.containsKey(name))) {
				...
				Firis.logger.warn("[" + this.name + "] Duplicate uniform: " + type.toString().toLowerCase() + " " + name);
		...
		public ProgramUniforms buildUniforms() {
			...
			for (int index = 0; index < activeUniforms; index++) {
				String name = FirisRenderSystem.getActiveUniform(program, index, 128, sizeBuf, typeBuf);
				...
				if (provided != null && provided != expected) {
					...
					Firis.logger.error("[" + this.name + "] Wrong uniform type for " + name + ": Firis is providing " + provided + " but the program expects " + expectedName + ". Disabling that uniform.");
