/*
 * Copyright LWJGL. All rights reserved. Modified by IMS for use in Firis (net.coderbot.iris.gl).
...
public final class GLDebug {
	...
	public static int setupDebugMessageCallback(PrintStream stream) {
		...
		if (caps.OpenGL43) {
			Firis.logger.info("[GL] Using OpenGL 4.3 for error logging.");
			...
			if ((GL43C.glGetInteger(33310) & 2) == 0) {
				Firis.logger.warn("[GL] Warning: A non-debug context may not produce any debug output.");
				GL43C.glEnable(37600);
				return 2;
			}
			return 1;
		} else if (caps.GL_KHR_debug) {
			Firis.logger.info("[GL] Using KHR_debug for error logging.");
			GLDebugMessageCallback proc = GLDebugMessageCallback.create((source, type, id, severity, length, message, userParam) -> {
				stream.println("[LWJGL] OpenGL debug message");
				printDetail(stream, "ID", String.format("0x%X", id));
				printDetail(stream, "Source", getDebugSource(source));
			...
			Firis.logger.info("[GL] Using ARB_debug_output for error logging.");
			...
			Firis.logger.info("[GL] Using AMD_debug_output for error logging.");
			...
			Firis.logger.info("[GL] No debug output implementation is available, cannot return debug info.");
			return 0;
		}
	}
	...
	public static int disableDebugMessages() {
		...
		if (caps.OpenGL43) {
			...
			Firis.logger.info("[GL] No debug output implementation is available, cannot disable debug info.");
			return 0;
		}
	}
	...
	public static void reloadDebugState() {
		if (Firis.getFirisConfig().areDebugOptionsEnabled() && (GL.getCapabilities().GL_KHR_debug || GL.getCapabilities().OpenGL43)) {
