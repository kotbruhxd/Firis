package net.irisshaders.firis.gl.program;
...
public final class ComputeProgram extends GlResource {
	...
	ComputeProgram(int program, ProgramUniforms uniforms, ProgramSamplers samplers, ProgramImages images) {
		...
		localSize = new int[3];
		FirisRenderSystem.getProgramiv(program, GL43C.GL_COMPUTE_WORK_GROUP_SIZE, localSize);
	...
	public void dispatch(float width, float height) {
		if (!Firis.getPipelineManager().getPipeline().map(WorldRenderingPipeline::allowConcurrentCompute).orElse(false)) {
		...
		if (indirectPointer != null) {
			FirisRenderSystem.bindBuffer(GL46C.GL_DISPATCH_INDIRECT_BUFFER, indirectPointer.buffer());
			FirisRenderSystem.dispatchComputeIndirect(indirectPointer.offset());
		} else {
			FirisRenderSystem.dispatchCompute(getWorkGroups(width, height));
		}
	}
