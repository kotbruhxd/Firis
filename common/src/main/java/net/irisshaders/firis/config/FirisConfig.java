package net.irisshaders.firis.config;
...
public class FirisConfig {
	private static final String COMMENT =
		"This file stores configuration options for Firis, such as the currently active shaderpack";
	...
	public FirisConfig(Path propertiesPath) {
		shaderPackName = null;
		enableShaders = true;
	...
	public void load() throws IOException {
		...
		try {
			FirisVideoSettings.shadowDistance = Integer.parseInt(properties.getProperty("maxShadowRenderDistance", "32"));
			FirisVideoSettings.colorSpace = ColorSpace.valueOf(properties.getProperty("colorSpace", "SRGB"));
		} catch (IllegalArgumentException e) {
			Firis.logger.error("Shadow distance setting reset; value is invalid.");
			FirisVideoSettings.shadowDistance = 32;
			FirisVideoSettings.colorSpace = ColorSpace.SRGB;
			save();
		}
	...
	public void save() throws IOException {
		...
		properties.setProperty("maxShadowRenderDistance", String.valueOf(FirisVideoSettings.shadowDistance));
		properties.setProperty("colorSpace", FirisVideoSettings.colorSpace.name());
		// NB: This uses ISO-8859-1 with unicode escapes as the encoding
