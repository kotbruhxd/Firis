package net.irisshaders.firis.compat.indigo.mixin;
...
public class MixinAbstractBlockRenderContext {
	...
	private static int firis$multiplyRGB(int color, float shade) {
	...
	private int firis$separateAoColorMultiply(int color, float ao) {
		if (WorldRenderingSettings.INSTANCE.shouldUseSeparateAo()) {
			...
			return firis$multiplyRGB(color, ao);
		}
	}
}
