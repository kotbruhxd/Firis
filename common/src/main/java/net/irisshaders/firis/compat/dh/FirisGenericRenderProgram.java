...
import net.irisshaders.firis.gl.shader.GlShader;
import net.irisshaders.firis.gl.shader.ShaderType;
...
import net.irisshaders.firis.uniforms.builtin.BuiltinReplacementUniforms;
...
public class FirisGenericRenderProgram implements IDhApiGenericObjectShaderProgram {
	// Uniforms
	...
	private FirisGenericRenderProgram(String name, boolean isShadowPass, boolean translucent, BlendModeOverride override, BufferBlendOverride[] bufferBlendOverrides, String vertex, String tessControl, String tessEval, String geometry, String fragment, CustomUniforms customUniforms, FirisRenderingPipeline pipeline) {
		...
		if (status != 1) {
			String message = "Shader link error in Firis DH program! Details: " + GL32.glGetProgramInfoLog(this.id);
			this.free();
		...
		ProgramSamplers.Builder samplerBuilder = ProgramSamplers.builder(id, FirisSamplers.WORLD_RESERVED_TEXTURE_UNITS);
	...
	public static FirisGenericRenderProgram createProgram(String name, boolean isShadowPass, boolean translucent, ProgramSource source, CustomUniforms uniforms, FirisRenderingPipeline pipeline) {
		...
		return new FirisGenericRenderProgram(name, isShadowPass, translucent, source.getDirectives().getBlendModeOverride().orElse(null), bufferOverrides.toArray(BufferBlendOverride[]::new), vertex, tessControl, tessEval, geometry, fragment, uniforms, pipeline);
	}
	...
	public boolean overrideThisFrame() {
		return FirisApi.getInstance().isShaderPackInUse();
	}
	...
	public void fillSharedDirectUniformData(DhApiRenderParam dhApiRenderParam, DhApiRenderableBoxGroupShading dhApiRenderableBoxGroupShading, IDhApiRenderableBoxGroup iDhApiRenderableBoxGroup, DhApiVec3d dhApiVec3d) {
		throw new IllegalStateException("Only indirect is supported with Firis.");
	}
