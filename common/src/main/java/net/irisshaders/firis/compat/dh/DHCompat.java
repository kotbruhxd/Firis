package net.irisshaders.firis.compat.dh;
...
public class DHCompat {
	...
	public DHCompat(FirisRenderingPipeline pipeline, boolean renderDHShadow) {
		try {
			if (dhPresent) {
				compatInternalInstance = Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal").getDeclaredConstructor(pipeline.getClass(), boolean.class).newInstance(pipeline, renderDHShadow);
	...
	public static void run() {
		try {
			if (FirisPlatformHelpers.getInstance().isModLoaded("distanthorizons")) {
				deletePipeline = MethodHandles.lookup().findVirtual(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "clear", MethodType.methodType(void.class));
				setupEventHandlers = MethodHandles.lookup().findStatic(Class.forName("net.irisshaders.firis.compat.dh.LodRendererEvents"), "setupEventHandlers", MethodType.methodType(void.class));
				getDepthTex = MethodHandles.lookup().findVirtual(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "getStoredDepthTex", MethodType.methodType(int.class));
				getRenderDistance = MethodHandles.lookup().findStatic(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "getRenderDistance", MethodType.methodType(int.class));
				incompatible = MethodHandles.lookup().findVirtual(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "incompatiblePack", MethodType.methodType(boolean.class));
				getFarPlane = MethodHandles.lookup().findStatic(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "getFarPlane", MethodType.methodType(float.class));
				getNearPlane = MethodHandles.lookup().findStatic(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "getNearPlane", MethodType.methodType(float.class));
				getDepthTexNoTranslucent = MethodHandles.lookup().findVirtual(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "getDepthTexNoTranslucent", MethodType.methodType(int.class));
				checkFrame = MethodHandles.lookup().findStatic(Class.forName("net.irisshaders.firis.compat.dh.DHCompatInternal"), "checkFrame", MethodType.methodType(boolean.class));
			...
			if (FirisPlatformHelpers.getInstance().isModLoaded("distanthorizons")) {
				if (e instanceof ExceptionInInitializerError eiie) {
					...
					throw new RuntimeException("DH found, but one or more API methods are missing. Firis requires DH [2.0.4] or DH API version [1.1.0] or newer. Please make sure you are on the latest version of DH and Firis.", e);
				}
			} else {
				Firis.logger.info("DH not found, and classes not found.");
			}
		}
	}
