...
import net.irisshaders.firis.gl.texture.DepthCopyStrategy;
...
import net.irisshaders.firis.targets.DepthTexture;
...
public class DHCompatInternal {
	...
	private final FirisRenderingPipeline pipeline;
	private GlFramebuffer dhGenericFramebuffer;
	public boolean shouldOverrideShadow;
	public boolean shouldOverride;
	private FirisLodRenderProgram solidProgram;
	private FirisGenericRenderProgram genericShader;
	private FirisLodRenderProgram translucentProgram;
	private FirisLodRenderProgram shadowProgram;
	...
	public DHCompatInternal(FirisRenderingPipeline pipeline, boolean dhShadowEnabled) {
		...
		if (pipeline.getDHTerrainShader().isEmpty() && pipeline.getDHWaterShader().isEmpty()) {
			Firis.logger.warn("No DH shader found in this pack.");
			incompatible = true;
			return;
		}

		cachedVersion = ((Blaze3dRenderTargetExt) Minecraft.getInstance().getMainRenderTarget()).firis$getDepthBufferVersion();
		...
		solidProgram = FirisLodRenderProgram.createProgram(terrain.getName(), false, false, terrain, pipeline.getCustomUniforms(), pipeline);

		ProgramSource generic = pipeline.getDHGenericShader().get();
		genericShader = FirisGenericRenderProgram.createProgram(generic.getName() + "_g", false, false, generic, pipeline.getCustomUniforms(), pipeline);
		...
		if (pipeline.getDHWaterShader().isPresent()) {
			...
			translucentProgram = FirisLodRenderProgram.createProgram(water.getName(), false, true, water, pipeline.getCustomUniforms(), pipeline);
		...
		if (pipeline.getDHShadowShader().isPresent() && dhShadowEnabled) {
			...
			shadowProgram = FirisLodRenderProgram.createProgram(shadow.getName(), true, false, shadow, pipeline.getCustomUniforms(), pipeline);
	...
	public static boolean checkFrame() {
		...
		if ((dhEnabled != DhApi.Delayed.configs.graphics().renderingEnabled().getValue() || guiScale != Minecraft.getInstance().options.guiScale().get())
			&& FirisApi.getInstance().isShaderPackInUse()) {
			guiScale = Minecraft.getInstance().options.guiScale().get();
	...
	public void reconnectDHTextures(int depthTex) {
		if (((Blaze3dRenderTargetExt) Minecraft.getInstance().getMainRenderTarget()).firis$getDepthBufferVersion() != cachedVersion) {
			cachedVersion = ((Blaze3dRenderTargetExt) Minecraft.getInstance().getMainRenderTarget()).firis$getDepthBufferVersion();
			createDepthTex(Minecraft.getInstance().getMainRenderTarget().width, Minecraft.getInstance().getMainRenderTarget().height);
		}
	...
	public FirisLodRenderProgram getSolidShader() {
		return solidProgram;
	}
	...
	public FirisLodRenderProgram getShadowShader() {
	...
	public FirisLodRenderProgram getTranslucentShader() {
		if (translucentProgram == null) {
			return solidProgram;
		}
	...
	public void copyTranslucents(int width, int height) {
		if (translucentDepthDirty) {
			...
			FirisRenderSystem.copyTexImage2D(GL20C.GL_TEXTURE_2D, 0, DepthBufferFormat.DEPTH32F.getGlInternalFormat(), 0, 0, width, height, 0);
		} else {
