...
public class LodRendererEvents {
	...
	public static void setupEventHandlers() {
		if (!eventHandlersBound) {
			...
			Firis.logger.info("Queuing DH event binding...");
			...
			DhApiAfterDhInitEvent beforeCleanupEvent = new DhApiAfterDhInitEvent() {
				...
				public void afterDistantHorizonsInit(DhApiEventParam<Void> event) {
					Firis.logger.info("DH Ready, binding Firis event handlers...");
					...
					Firis.logger.info("DH Firis events bound.");
				}
			};
	...
	private static void setupSetDeferredBeforeRenderingEvent() {
		DhApiBeforeRenderEvent beforeRenderEvent = new DhApiBeforeRenderEvent() {
			...
			public void beforeRender(DhApiCancelableEventParam<DhApiRenderParam> event) {
				...
				DhApi.Delayed.renderProxy.setDeferTransparentRendering(FirisApi.getInstance().isShaderPackInUse() && getInstance().shouldOverride);
	...
	private static DHCompatInternal getInstance() {
		return (DHCompatInternal) Firis.getPipelineManager().getPipeline().map(WorldRenderingPipeline::getDHCompat).map(DHCompat::getInstance).orElse(DHCompatInternal.SHADERLESS);
	}
			...
			public void beforeRender(DhApiEventParam<DhApiRenderParam> event) {
				...
				if (instance.shouldOverride) {
					...
					if (event.value.renderPass == EDhApiRenderPass.OPAQUE_AND_TRANSPARENT) {
						Firis.logger.error("Unexpected; somehow the Opaque + Translucent pass ran with shaders on.");
					}
				} else {
				...
				if (event.value.renderPass == EDhApiRenderPass.OPAQUE) {
					...
					if (instance.shouldOverride) {
						if (ShadowRenderingState.areShadowsCurrentlyBeingRendered()) {
							...
							//Firis.logger.info("event near clip: "+event.value.nearClipPlane+" event far clip: "+event.value.farClipPlane+
