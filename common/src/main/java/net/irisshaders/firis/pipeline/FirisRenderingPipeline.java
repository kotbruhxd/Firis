...
import net.irisshaders.firis.gl.image.GlImage;
import net.irisshaders.firis.gl.image.ImageHolder;
import net.irisshaders.firis.gl.program.ComputeProgram;
import net.irisshaders.firis.gl.program.ProgramBuilder;
...
import net.irisshaders.firis.gl.sampler.SamplerLimits;
import net.irisshaders.firis.gl.shader.ShaderCompileException;
import net.irisshaders.firis.gl.state.FogMode;
import net.irisshaders.firis.gl.state.ShaderAttributeInputs;
import net.irisshaders.firis.gl.texture.DepthBufferFormat;
...
import net.irisshaders.firis.helpers.OptionalBoolean;
import net.irisshaders.firis.helpers.Tri;
import net.irisshaders.firis.mixin.GlStateManagerAccessor;
import net.irisshaders.firis.mixin.LevelRendererAccessor;
import net.irisshaders.firis.pathways.CenterDepthSampler;
import net.irisshaders.firis.pathways.HorizonRenderer;
import net.irisshaders.firis.pathways.colorspace.ColorSpace;
import net.irisshaders.firis.pathways.colorspace.ColorSpaceConverter;
import net.irisshaders.firis.pathways.colorspace.ColorSpaceFragmentConverter;
import net.irisshaders.firis.pipeline.programs.ExtendedShader;
import net.irisshaders.firis.pipeline.programs.FallbackShader;
import net.irisshaders.firis.pipeline.programs.ShaderCreator;
import net.irisshaders.firis.pipeline.programs.ShaderKey;
import net.irisshaders.firis.pipeline.programs.ShaderMap;
import net.irisshaders.firis.pipeline.transform.PatchShaderType;
import net.irisshaders.firis.pipeline.transform.ShaderPrinter;
import net.irisshaders.firis.pipeline.transform.TransformPatcher;
import net.irisshaders.firis.samplers.FirisImages;
import net.irisshaders.firis.samplers.FirisSamplers;
import net.irisshaders.firis.shaderpack.FilledIndirectPointer;
import net.irisshaders.firis.shaderpack.ImageInformation;
import net.irisshaders.firis.shaderpack.ShaderPack;
import net.irisshaders.firis.shaderpack.loading.ProgramArrayId;
import net.irisshaders.firis.shaderpack.loading.ProgramId;
...
import net.irisshaders.firis.shaderpack.programs.ComputeSource;
import net.irisshaders.firis.shaderpack.programs.ProgramFallbackResolver;
import net.irisshaders.firis.shaderpack.programs.ProgramSet;
import net.irisshaders.firis.shaderpack.programs.ProgramSource;
import net.irisshaders.firis.shaderpack.properties.CloudSetting;
import net.irisshaders.firis.shaderpack.properties.PackDirectives;
import net.irisshaders.firis.shaderpack.properties.PackShadowDirectives;
...
import net.irisshaders.firis.shadows.ShadowCompositeRenderer;
import net.irisshaders.firis.shadows.ShadowRenderTargets;
import net.irisshaders.firis.shadows.ShadowRenderer;
import net.irisshaders.firis.targets.Blaze3dRenderTargetExt;
import net.irisshaders.firis.targets.BufferFlipper;
import net.irisshaders.firis.targets.ClearPass;
import net.irisshaders.firis.targets.ClearPassCreator;
import net.irisshaders.firis.targets.RenderTargetStateListener;
import net.irisshaders.firis.targets.RenderTargets;
import net.irisshaders.firis.targets.backed.NativeImageBackedSingleColorTexture;
import net.irisshaders.firis.texture.TextureInfoCache;
import net.irisshaders.firis.texture.format.TextureFormat;
import net.irisshaders.firis.texture.format.TextureFormatLoader;
import net.irisshaders.firis.texture.pbr.PBRTextureHolder;
import net.irisshaders.firis.texture.pbr.PBRTextureManager;
import net.irisshaders.firis.texture.pbr.PBRType;
import net.irisshaders.firis.uniforms.CapturedRenderingState;
import net.irisshaders.firis.uniforms.CommonUniforms;
...
public class FirisRenderingPipeline implements WorldRenderingPipeline, ShaderRenderingPipeline, RenderTargetStateListener {
	...
	public FirisRenderingPipeline(ProgramSet programSet) {
		...
		if (!programSet.getPackDirectives().getBufferObjects().isEmpty()) {
			if (FirisRenderSystem.supportsSSBO()) {
			...
			for (int i = 0; i < Math.min(16, SamplerLimits.get().getMaxShaderStorageUnits()); i++) {
				FirisRenderSystem.bindBufferBase(GL43C.GL_SHADER_STORAGE_BUFFER, i, 0);
			}
		}
		...
		this.renderTargets = new RenderTargets(main.width, main.height, depthTextureId, ((Blaze3dRenderTargetExt) main).firis$getDepthBufferVersion(), depthBufferFormat, programSet.getPackDirectives().getRenderTargetDirectives().getRenderTargetSettings(), programSet.getPackDirectives());
		...
		IntFunction<ProgramSamplers> createTerrainSamplers = (programId) -> {
			ProgramSamplers.Builder builder = ProgramSamplers.builder(programId, FirisSamplers.WORLD_RESERVED_TEXTURE_UNITS);
			...
			FirisSamplers.addRenderTargetSamplers(customTextureSamplerInterceptor, flipped, renderTargets, false, this);
			FirisSamplers.addCustomTextures(builder, customTextureManager.getIrisCustomTextures());

			if (!shouldBindPBR) {
				shouldBindPBR = FirisSamplers.hasPBRSamplers(customTextureSamplerInterceptor);
			}

			FirisSamplers.addLevelSamplers(customTextureSamplerInterceptor, this, whitePixel, true, true, false);
			FirisSamplers.addWorldDepthSamplers(customTextureSamplerInterceptor, renderTargets);
			FirisSamplers.addNoiseSampler(customTextureSamplerInterceptor, customTextureManager.getNoiseTexture());
			FirisSamplers.addCustomImages(customTextureSamplerInterceptor, customImages);

			if (FirisSamplers.hasShadowSamplers(customTextureSamplerInterceptor)) {
				// we compiled the non-Sodium version of this program first... so if this is somehow null, something
				// very odd is going on.
				FirisSamplers.addShadowSamplers(customTextureSamplerInterceptor, Objects.requireNonNull(shadowRenderTargets), null, separateHardwareSamplers);
			}
		...
		IntFunction<ProgramImages> createTerrainImages = (programId) -> {
			...
			FirisImages.addRenderTargetImages(builder, flipped, renderTargets);
			FirisImages.addCustomImages(builder, customImages);

			if (FirisImages.hasShadowImages(builder)) {
				// we compiled the non-Sodium version of this program first... so if this is somehow null, something
				// very odd is going on.
				FirisImages.addShadowColorImages(builder, Objects.requireNonNull(shadowRenderTargets), null);
			}
		...
		IntFunction<ProgramSamplers> createShadowTerrainSamplers = (programId) -> {
			...
			FirisSamplers.addRenderTargetSamplers(customTextureSamplerInterceptor, () -> flippedBeforeShadow, renderTargets, false, this);
			FirisSamplers.addCustomTextures(builder, customTextureManager.getIrisCustomTextures());
		...
		IntFunction<ProgramImages> createShadowTerrainImages = (programId) -> {
			...
			FirisImages.addRenderTargetImages(builder, () -> flippedBeforeShadow, renderTargets);
			FirisImages.addCustomImages(builder, customImages);
		...
		if (programSet.getPackDirectives().supportsColorCorrection()) {
			...
			//if (FirisRenderSystem.supportsCompute()) {
			//	colorSpaceConverter = new ColorSpaceComputeConverter(main.width, main.height, FirisVideoSettings.colorSpace);
			//} else {
			colorSpaceConverter = new ColorSpaceFragmentConverter(main.width, main.height, FirisVideoSettings.colorSpace);
			//}
		}

		currentColorSpace = FirisVideoSettings.colorSpace;
	}
		...
		for (int i = 0; i < programs.length; i++) {
			...
			if (source == null || !source.getSource().isPresent()) {
				...
				try {
					...
					builder = ProgramBuilder.beginCompute(source.getName(), transformed, FirisSamplers.WORLD_RESERVED_TEXTURE_UNITS);
				...
				if (FirisSamplers.hasShadowSamplers(customTextureSamplerInterceptor)) {
					if (shadowRenderTargets != null) {
						FirisSamplers.addShadowSamplers(customTextureSamplerInterceptor, shadowRenderTargets, null, separateHardwareSamplers);
						FirisImages.addShadowColorImages(builder, shadowRenderTargets, null);
					}
				}
		...
		for (int i = 0; i < programs.length; i++) {
			...
			if (source == null || !source.getSource().isPresent()) {
				...
				try {
					...
					builder = ProgramBuilder.beginCompute(source.getName(), transformed, FirisSamplers.COMPOSITE_RESERVED_TEXTURE_UNITS);
				} catch (RuntimeException e) {
					// TODO: Better error handling
				...
				FirisSamplers.addRenderTargetSamplers(customTextureSamplerInterceptor, flipped, renderTargets, true, this);
				FirisSamplers.addCustomTextures(builder, customTextureManager.getIrisCustomTextures());
				FirisSamplers.addCompositeSamplers(builder, renderTargets);
				FirisSamplers.addCustomImages(customTextureSamplerInterceptor, customImages);
				FirisImages.addRenderTargetImages(builder, flipped, renderTargets);
				FirisImages.addCustomImages(builder, customImages);
	...
	public void addGbufferOrShadowSamplers(SamplerHolder samplers, ImageHolder images, Supplier<ImmutableSet<Integer>> flipped,
		...
		FirisSamplers.addRenderTargetSamplers(samplerHolder, flipped, renderTargets, false, this);
		FirisSamplers.addCustomTextures(samplerHolder, customTextureManager.getIrisCustomTextures());
		FirisImages.addRenderTargetImages(images, flipped, renderTargets);
		FirisImages.addCustomImages(images, customImages);

		if (!shouldBindPBR) {
			shouldBindPBR = FirisSamplers.hasPBRSamplers(samplerHolder);
		}

		FirisSamplers.addLevelSamplers(samplers, this, whitePixel, hasTexture, hasLightmap, hasOverlay);
		FirisSamplers.addWorldDepthSamplers(samplerHolder, this.renderTargets);
		FirisSamplers.addNoiseSampler(samplerHolder, this.customTextureManager.getNoiseTexture());
		FirisSamplers.addCustomImages(samplerHolder, customImages);

		if (FirisSamplers.hasShadowSamplers(samplerHolder)) {
			FirisSamplers.addShadowSamplers(samplerHolder, shadowTargetsSupplier.get(), null, separateHardwareSamplers);
		}

		if (isShadowPass || FirisImages.hasShadowImages(images)) {
			FirisImages.addShadowColorImages(images, shadowTargetsSupplier.get(), null);
		}
	}
	...
	public void beginLevelRendering() {
		...
		boolean changed = renderTargets.resizeIfNeeded(((Blaze3dRenderTargetExt) main).firis$getDepthBufferVersion(), depthTextureId, main.width,
		...
		if (changed || FirisVideoSettings.colorSpace != currentColorSpace) {
			currentColorSpace = FirisVideoSettings.colorSpace;
	...
	public void addDebugText(List<String> messages) {
		if (this.shadowRenderer != null) {
			...
			messages.add("");
			messages.add("[Firis] Shadow Maps: not used by shader pack");
		}
	}
	...
	public void destroy() {
		...
		for (int i = 0; i < 16; i++) {
			...
			FirisRenderSystem.unbindAllSamplers();
