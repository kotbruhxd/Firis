package net.irisshaders.firis.pipeline.transform.transformer;
...
public class CompatibilityTransformer {
	...
	public static void transformEach(ASTParser t, TranslationUnit tree, Root root, Parameters parameters) {
		if (parameters.type == PatchShaderType.VERTEX) {
			if (root.replaceExpressionMatches(t, sildursWaterFract, "fract(worldpos.y + 0.01)")) {
				Firis.logger.warn("Patched fract(worldpos.y + 0.001) to fract(worldpos.y + 0.01) to fix " +
					"waving water disconnecting from other water blocks; See https://github.com/FirisShaders/Firis/issues/509");
			}
		}
	...
	public static void transformGrouped(
		...
		/*
		  ...
		  TODO:
		  - fix issues where Firis' own declarations are detected and patched like
