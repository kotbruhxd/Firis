...
import net.irisshaders.firis.gl.program.Program;
...
import net.irisshaders.firis.gl.texture.TextureAccess;
import net.irisshaders.firis.mixin.GlStateManagerAccessor;
import net.irisshaders.firis.pathways.CenterDepthSampler;
import net.irisshaders.firis.pathways.FullScreenQuadRenderer;
...
import net.irisshaders.firis.shaderpack.properties.PackRenderTargetDirectives;
import net.irisshaders.firis.shaderpack.properties.ProgramDirectives;
...
import net.irisshaders.firis.targets.RenderTarget;
...
public class FinalPassRenderer {
	...
	public FinalPassRenderer(WorldRenderingPipeline pipeline, ProgramSet pack, RenderTargets renderTargets, TextureAccess noiseTexture, ShaderStorageBufferHolder holder,
		...
		this.lastColorTextureVersion = ((Blaze3dRenderTargetExt) Minecraft.getInstance().getMainRenderTarget()).firis$getColorBufferVersion();
	...
	private static void setupMipmapping(RenderTarget target, boolean readFromAlt) {
		...
		FirisRenderSystem.generateMipmaps(texture, GL20C.GL_TEXTURE_2D);
		...
		FirisRenderSystem.texParameteri(texture, GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MIN_FILTER, filter);
	}
	...
	private static void resetRenderTarget(RenderTarget target) {
		...
		FirisRenderSystem.texParameteri(target.getMainTexture(), GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MIN_FILTER, filter);
		FirisRenderSystem.texParameteri(target.getAltTexture(), GL20C.GL_TEXTURE_2D, GL20C.GL_TEXTURE_MIN_FILTER, filter);
	...
	public void renderFinalPass() {
		...
		if (((Blaze3dRenderTargetExt) main).firis$getColorBufferVersion() != lastColorTextureVersion || main.getColorTextureId() != lastColorTextureId) {
			lastColorTextureVersion = ((Blaze3dRenderTargetExt) main).firis$getColorBufferVersion();
		...
		if (this.finalPass != null) {
			...
			FirisRenderSystem.memoryBarrier(GL43C.GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL43C.GL_TEXTURE_FETCH_BARRIER_BIT | GL43C.GL_SHADER_STORAGE_BARRIER_BIT);
			...
			// If there are no passes, we somehow need to transfer the content of the Firis color render targets into
			...
			FirisRenderSystem.copyTexSubImage2D(main.getColorTextureId(), GL11C.GL_TEXTURE_2D, 0, 0, 0, 0, 0, baseWidth, baseHeight);
		}
	...
	private Program createProgram(ProgramSource source, ImmutableSet<Integer> flipped, ImmutableSet<Integer> flippedAtLeastOnceSnapshot,
		...
		try {
			builder = ProgramBuilder.begin(source.getName(), vertex, geometry, fragment,
				FirisSamplers.COMPOSITE_RESERVED_TEXTURE_UNITS);
		...
		FirisSamplers.addRenderTargetSamplers(customTextureSamplerInterceptor, () -> flipped, renderTargets, true, pipeline);
		...
		FirisSamplers.addCustomTextures(builder, irisCustomTextures);
		FirisSamplers.addNoiseSampler(customTextureSamplerInterceptor, noiseTexture);
		FirisSamplers.addCompositeSamplers(customTextureSamplerInterceptor, renderTargets);

		if (FirisSamplers.hasShadowSamplers(customTextureSamplerInterceptor)) {
			FirisSamplers.addShadowSamplers(customTextureSamplerInterceptor, shadowTargetsSupplier.get(), null, pipeline.hasFeature(FeatureFlags.SEPARATE_HARDWARE_SAMPLERS));
			FirisImages.addShadowColorImages(builder, shadowTargetsSupplier.get(), null);
		}
