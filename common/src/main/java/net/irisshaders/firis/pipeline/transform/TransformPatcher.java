package net.irisshaders.firis.pipeline.transform;
...
import net.irisshaders.firis.gl.FirisLimits;
...
import net.irisshaders.firis.pipeline.transform.parameter.ComputeParameters;
import net.irisshaders.firis.pipeline.transform.parameter.Parameters;
import net.irisshaders.firis.pipeline.transform.parameter.SodiumParameters;
import net.irisshaders.firis.pipeline.transform.parameter.TextureStageParameters;
import net.irisshaders.firis.pipeline.transform.parameter.VanillaParameters;
import net.irisshaders.firis.pipeline.transform.transformer.CommonTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.CompatibilityTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.CompositeCoreTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.CompositeTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.DHGenericTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.DHTerrainTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.LayoutTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.SodiumCoreTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.SodiumTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.TextureTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.VanillaCoreTransformer;
import net.irisshaders.firis.pipeline.transform.transformer.VanillaTransformer;
...
public class TransformPatcher {
	...
	static {
		...
		transformer.setTransformation((trees, parameters) -> {
			for (PatchShaderType type : PatchShaderType.values()) {
				...
				// check for illegal references to internal Firis shader interfaces
				internalPrefixes.stream()
					.flatMap(root.getPrefixIdentifierIndex()::prefixQueryFlat)
					.findAny()
					.ifPresent(id -> {
						throw new IllegalArgumentException(
							"Detected a potential reference to unstable and internal Firis shader interfaces (iris_, irisMain and moj_import). This isn't currently supported. Violation: "
	...
	private static Map<PatchShaderType, String> transform(String name, String vertex, String geometry, String tessControl, String tessEval, String fragment,
		...
		if (result == null) {
			transformer.setPrintType(Firis.getFirisConfig().areDebugOptionsEnabled() ? PrintType.INDENTED : PrintType.SIMPLE);
