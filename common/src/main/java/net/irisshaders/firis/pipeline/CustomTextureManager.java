...
import net.irisshaders.firis.targets.backed.NativeImageBackedNoiseTexture;
	...
	public CustomTextureManager(PackDirectives packDirectives,
		...
		customTextureDataMap.forEach((textureStage, customTextureStageDataMap) -> {
			...
			customTextureStageDataMap.forEach((samplerName, textureData) -> {
				try {
					...
					Firis.logger.error("Unable to parse the image data for the custom texture on stage "
		...
		irisCustomTextureDataMap.forEach((name, texture) -> {
			try {
				...
				Firis.logger.error("Unable to parse the image data for the custom texture on sampler " + name, e);
			}
		});
		...
		noise = customNoiseTextureData.flatMap(textureData -> {
			try {
				...
				Firis.logger.error("Unable to parse the image data for the custom noise texture", e);
