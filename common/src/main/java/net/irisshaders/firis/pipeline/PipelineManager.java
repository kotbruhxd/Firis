...
public class PipelineManager {
	...
	public WorldRenderingPipeline preparePipeline(NamespacedId currentDimension) {
		if (!pipelinesPerDimension.containsKey(currentDimension)) {
			...
			Firis.logger.info("Creating pipeline for dimension {}", currentDimension);
	...
	/**
	 * In FirisChunkProgramOverrides#getProgramOverride,
	 ...
	 * and every dimension corresponds to a FirisChunkProgramOverrides object.
	 ...
	 * <p>In particular, </p>
	 *
	 * @see <a href="https://github.com/FirisShaders/Firis/issues/1330">this GitHub issue</a>
	 */
	public void destroyPipeline() {
		pipelinesPerDimension.forEach((dimensionId, pipeline) -> {
			Firis.logger.info("Destroying pipeline {}", dimensionId);
			resetTextureState();
			pipeline.destroy();
		});
