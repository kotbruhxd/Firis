...
import net.irisshaders.firis.gl.uniform.UniformUpdateFrequency;
import net.irisshaders.firis.gui.option.FirisVideoSettings;
import net.irisshaders.firis.helpers.JomlConversions;
import net.irisshaders.firis.mixin.GameRendererAccessor;
...
public class FirisExclusiveUniforms {
	private static final Vector3d ZERO = new Vector3d(0);

	public static void addFirisExclusiveUniforms(UniformHolder uniforms) {
		WorldInfoUniforms.addWorldInfoUniforms(uniforms);

		uniforms.uniform1i(UniformUpdateFrequency.PER_TICK, "currentColorSpace", () -> FirisVideoSettings.colorSpace.ordinal());

		//All Firis-exclusive uniforms (uniforms which do not exist in either OptiFine or ShadersMod) should be registered here.
		uniforms.uniform1f(UniformUpdateFrequency.PER_FRAME, "thunderStrength", FirisExclusiveUniforms::getThunderStrength);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "currentPlayerHealth", FirisExclusiveUniforms::getCurrentHealth);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "maxPlayerHealth", FirisExclusiveUniforms::getMaxHealth);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "currentPlayerHunger", FirisExclusiveUniforms::getCurrentHunger);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "maxPlayerHunger", () -> 20);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "currentPlayerArmor", FirisExclusiveUniforms::getCurrentArmor);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "maxPlayerArmor", () -> 50);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "currentPlayerAir", FirisExclusiveUniforms::getCurrentAir);
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "maxPlayerAir", FirisExclusiveUniforms::getMaxAir);
		uniforms.uniform1b(UniformUpdateFrequency.PER_FRAME, "firstPersonCamera", FirisExclusiveUniforms::isFirstPersonCamera);
		uniforms.uniform1b(UniformUpdateFrequency.PER_TICK, "isSpectator", FirisExclusiveUniforms::isSpectator);
		uniforms.uniform1i(PER_FRAME, "currentSelectedBlockId", FirisExclusiveUniforms::getCurrentSelectedBlockId);
		uniforms.uniform3f(PER_FRAME, "currentSelectedBlockPos", FirisExclusiveUniforms::getCurrentSelectedBlockPos);
		uniforms.uniform3d(UniformUpdateFrequency.PER_FRAME, "eyePosition", FirisExclusiveUniforms::getEyePosition);
