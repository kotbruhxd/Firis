...
import net.irisshaders.firis.layer.GbufferPrograms;
import net.irisshaders.firis.mixin.GlStateManagerAccessor;
import net.irisshaders.firis.mixin.statelisteners.BooleanStateAccessor;
import net.irisshaders.firis.mixin.texture.TextureAtlasAccessor;
import net.irisshaders.firis.mixinterface.LocalPlayerInterface;
import net.irisshaders.firis.shaderpack.IdMap;
import net.irisshaders.firis.shaderpack.properties.PackDirectives;
import net.irisshaders.firis.texture.TextureInfoCache;
import net.irisshaders.firis.texture.TextureInfoCache.TextureInfo;
import net.irisshaders.firis.texture.TextureTracker;
import net.irisshaders.firis.uniforms.transforms.SmoothedFloat;
import net.irisshaders.firis.uniforms.transforms.SmoothedVec2f;
...
import static net.irisshaders.firis.gl.uniform.UniformUpdateFrequency.ONCE;
...
public final class CommonUniforms {
	...
	public static void addDynamicUniforms(DynamicUniformHolder uniforms, FogMode fogMode) {
		...
		FirisInternalUniforms.addFogUniforms(uniforms, fogMode);
	...
	public static void addNonDynamicUniforms(UniformHolder uniforms, IdMap idMap, PackDirectives directives, FrameUpdateNotifier updateNotifier) {
		...
		FirisExclusiveUniforms.addIrisExclusiveUniforms(uniforms);
		FirisTimeUniforms.addTimeUniforms(uniforms);
	...
	private static float getNightVision() {
		...
		if (cameraEntity instanceof LivingEntity livingEntity) {
			...
			try {
				// See MixinGameRenderer#firis$safecheckNightvisionStrength.
				//
