...
import net.irisshaders.firis.gl.uniform.LocationalUniformHolder;
import net.irisshaders.firis.gl.uniform.UniformHolder;
import net.irisshaders.firis.parsing.FirisFunctions;
import net.irisshaders.firis.parsing.FirisOptions;
import net.irisshaders.firis.parsing.VectorType;
	...
	private CustomUniforms(CustomUniformFixedInputUniformsHolder inputHolder, Map<String, Builder.Variable> variables) {
		...
		for (Builder.Variable variable : variables.values()) {
			try {
				...
				//Firis.logger.info("Was able to resolve uniform " + variable.name + " = " + variable.expression);
			} catch (Exception e) {
				Firis.logger
	...
	public static class Builder {
		...
		public void addVariable(String type, String name, String expression, boolean isUniform) {
			if (variables.containsKey(name)) {
				Firis.logger.warn("Ignoring duplicated custom uniform name: " + name);
				return;
			}

			Type parsedType = types.get(type);
			if (parsedType == null) {
				Firis.logger.warn("Ignoring invalid uniform type: " + type + " of " + name);
				return;
			}

			try {
				ExpressionElement ast = Parser.parse(expression, FirisOptions.options);
				variables.put(name, new Variable(parsedType, name, ast, isUniform));
			} catch (Exception e) {
				Firis.logger.warn("Failed to parse custom variable/uniform " + name + " with expression " + expression, e);
			}
		}
