package net.irisshaders.firis.uniforms.builtin;
...
public class BuiltinReplacementUniforms {
	...
	public static void addBuiltinReplacementUniforms(UniformHolder uniforms) {
		uniforms.uniformMatrix(UniformUpdateFrequency.ONCE, "iris_LightmapTextureMatrix", () -> {
			//Firis.logger.warn("A shader appears to require the lightmap texture matrix even after transformations have occurred");
			//Firis.logger.warn("Firis handles this correctly but it indicates that the shader is doing weird things with lightmap coordinates");
