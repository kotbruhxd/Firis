...
import net.irisshaders.firis.gl.shader.StandardMacros;
import net.irisshaders.firis.gui.debug.DebugLoadFailedGridScreen;
...
import net.irisshaders.firis.pipeline.PipelineManager;
...
import net.irisshaders.firis.shaderpack.discovery.ShaderpackDirectoryManager;
import net.irisshaders.firis.shaderpack.materialmap.NamespacedId;
import net.irisshaders.firis.shaderpack.option.OptionSet;
import net.irisshaders.firis.shaderpack.option.Profile;
...
public class Firis {
	public static final String MODID = "firis";
	 ...
	 */
	public static final String MODNAME = "Firis";
	public static final FirisLogging logger = new FirisLogging(MODNAME);
	...
	private static FirisConfig irisConfig;
	...
	private static String FIRIS_VERSION;
	private static UpdateChecker updateChecker;
	private static boolean fallback;

	static {
		if (!BuildConfig.ACTIVATE_RENDERDOC && FirisPlatformHelpers.getInstance().isDevelopmentEnvironment() && System.getProperty("user.name").contains("ims") && Util.getPlatform() == Util.OS.LINUX) {
	...
	public static void onRenderSystemInit() {
		if (!initialized) {
			Firis.logger.warn("Firis::onRenderSystemInit was called, but Firis::onEarlyInitialize was not called." +
	...
	public static void onLoadingComplete() {
		if (!initialized) {
			Firis.logger.warn("Firis::onLoadingComplete was called, but Firis::onEarlyInitialize was not called." +
				" Trying to avoid a crash but this is an odd state.");
			return;
		}
		...
		// See: https://github.com/FirisShaders/Firis/issues/323
		lastDimension = DimensionId.OVERWORLD;
		Firis.getPipelineManager().preparePipeline(DimensionId.OVERWORLD);
	}
		...
		if (reloadKeybind.consumeClick()) {
			try {
				...
				if (minecraft.player != null) {
					minecraft.player.displayClientMessage(Component.translatable("firis.shaders.reloaded"), false);
				}

			} catch (Exception e) {
				logger.error("Error while reloading Shaders for Firis!", e);

				if (minecraft.player != null) {
					minecraft.player.displayClientMessage(Component.translatable("firis.shaders.reloaded.failure", Throwables.getRootCause(e).getMessage()).withStyle(ChatFormatting.RED), false);
				}
			}
			...
			try {
				...
				if (minecraft.player != null) {
					minecraft.player.displayClientMessage(Component.translatable("firis.shaders.toggled.failure", Throwables.getRootCause(e).getMessage()).withStyle(ChatFormatting.RED), false);
				}
				setShadersDisabled();
				fallback = true;
			}
	...
	public static void toggleShaders(Minecraft minecraft, boolean enabled) throws IOException {
		...
		reload();
		if (minecraft.player != null) {
			minecraft.player.displayClientMessage(enabled ? Component.translatable("firis.shaders.toggled", currentPackName) : Component.translatable("firis.shaders.disabled"), false);
		}
	}
	...
	public static void loadShaderpack() {
		if (irisConfig == null) {
			if (!initialized) {
				throw new IllegalStateException("Firis::loadShaderpack was called, but Firis::onInitializeClient wasn't" +
					" called yet. How did this happen?");
			} else {
				throw new NullPointerException("Firis.irisConfig was null unexpectedly");
			}
		}

		if (!irisConfig.areShadersEnabled()) {
			logger.info("Shaders are disabled because enableShaders is set to false in firis.properties");
	...
	private static Optional<Path> loadExternalZipShaderpack(Path shaderpackPath) throws IOException {
		FileSystem zipSystem = FileSystems.newFileSystem(shaderpackPath, Firis.class.getClassLoader());
	...
	public static void setDebug(boolean enable) {
		...
		if (Minecraft.getInstance().player != null) {
			Minecraft.getInstance().player.displayClientMessage(Component.translatable(success != 0 ? (enable ? "firis.shaders.debug.enabled" : "firis.shaders.debug.disabled") : "firis.shaders.debug.failure"), false);
			if (success == 2) {
				Minecraft.getInstance().player.displayClientMessage(Component.translatable("firis.shaders.debug.restart"), false);
			}
		}
	}
	...
	public static boolean isValidShaderpack(Path pack) {
		...
		if (pack.toString().endsWith(".zip")) {
			try (FileSystem zipSystem = FileSystems.newFileSystem(pack, Firis.class.getClassLoader())) {
				Path root = zipSystem.getRootDirectories().iterator().next();
				try (Stream<Path> stream = Files.walk(root)) {
					return stream
						.filter(Files::isDirectory)
				...
				Firis.logger.warn("The ZIP at " + pack + " is corrupt");
			} catch (IOException ignored) {
				// ignored, not a valid shader pack.
			}
		}
	...
	public static void reload() throws IOException {
		...
		// https://github.com/FirisShaders/Firis/issues/1330
		if (Minecraft.getInstance().level != null) {
			Firis.getPipelineManager().preparePipeline(Firis.getCurrentDimension());
		}
	}
	...
	private static WorldRenderingPipeline createPipeline(NamespacedId dimensionId) {
		...
		try {
			return new FirisRenderingPipeline(programs);
		} catch (Exception e) {
	...
	public static PipelineManager getPipelineManager() {
		if (pipelineManager == null) {
			pipelineManager = new PipelineManager(Firis::createPipeline);
		}
	...
	public static Optional<Exception> getStoredError() {
		Optional<Exception> stored = Firis.storedError;
	...
	public static FirisConfig getFirisConfig() {
		return irisConfig;
	}
	...
	public static String getVersion() {
		if (FIRIS_VERSION == null) {
			return "Version info unknown!";
		}

		return FIRIS_VERSION;
	}
	...
	public static String getFormattedVersion() {
		...
		if (FirisPlatformHelpers.getInstance().isDevelopmentEnvironment()) {
	...
	public static Path getShaderpacksDirectory() {
		if (shaderpacksDirectory == null) {
			shaderpacksDirectory = FirisPlatformHelpers.getInstance().getGameDir().resolve("shaderpacks");
		}
	...
	public void onEarlyInitialize() {
		FIRIS_VERSION = FirisPlatformHelpers.getInstance().getVersion();

		updateChecker = new UpdateChecker(FIRIS_VERSION);

		reloadKeybind = FirisPlatformHelpers.getInstance().registerKeyBinding(new KeyMapping("firis.keybind.reload", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_R, "firis.keybinds"));
		toggleShadersKeybind = FirisPlatformHelpers.getInstance().registerKeyBinding(new KeyMapping("firis.keybind.toggleShaders", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_K, "firis.keybinds"));
		shaderpackScreenKeybind = FirisPlatformHelpers.getInstance().registerKeyBinding(new KeyMapping("firis.keybind.shaderPackSelection", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_O, "firis.keybinds"));
		wireframeKeybind = FirisPlatformHelpers.getInstance().registerKeyBinding(new KeyMapping("firis.keybind.wireframe", InputConstants.Type.KEYSYM, InputConstants.UNKNOWN.getValue(), "firis.keybinds"));
		...
		irisConfig = new FirisConfig(FirisPlatformHelpers.getInstance().getConfigDir().resolve("firis.properties"));

		try {
			irisConfig.initialize();
		} catch (IOException e) {
			logger.error("Failed to initialize Firis configuration, default values will be used instead");
			logger.error("", e);
		}
