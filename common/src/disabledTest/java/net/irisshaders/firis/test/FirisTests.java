package net.irisshaders.firis.test;
...
public class FirisTests {
	...
	public static ShaderPack loadPackOrFail(String name) {
		try {
			return new ShaderPack(FirisTests.getTestShaderPackPath(name), TEST_ENVIRONMENT_DEFINES);
		} catch (Exception e) {
	...
	public static Path getTestShaderPackPath(String name) {
		try {
			return Paths.get(FirisTests.class.getResource("/shaderpacks/" + name + "/shaders/").toURI());
