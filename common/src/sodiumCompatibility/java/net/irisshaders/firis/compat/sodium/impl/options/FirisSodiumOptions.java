package net.irisshaders.firis.compat.sodium.impl.options;

import net.caffeinemc.mods.fodium.client.gui.options.OptionFlag;
import net.caffeinemc.mods.fodium.client.gui.options.OptionImpact;
import net.caffeinemc.mods.fodium.client.gui.options.OptionImpl;
import net.caffeinemc.mods.fodium.client.gui.options.control.ControlValueFormatter;
import net.caffeinemc.mods.fodium.client.gui.options.control.CyclingControl;
import net.caffeinemc.mods.fodium.client.gui.options.control.SliderControl;
...
public class FirisSodiumOptions {
	public static OptionImpl<Options, Integer> createMaxShadowDistanceSlider(MinecraftOptionsStorage vanillaOpts) {
		OptionImpl<Options, Integer> maxShadowDistanceSlider = OptionImpl.createBuilder(int.class, vanillaOpts)
			...
			.setBinding((options, value) -> {
					FirisVideoSettings.shadowDistance = value;
					try {
				...
				options -> FirisVideoSettings.getOverriddenShadowDistance(FirisVideoSettings.shadowDistance))
			.setImpact(OptionImpact.HIGH)
			.setEnabled(true)
			.build();

		((OptionImplExtended) maxShadowDistanceSlider).firis$dynamicallyEnable(FirisVideoSettings::isShadowDistanceSliderEnabled);
	...
	public static OptionImpl<Options, ColorSpace> createColorSpaceButton(MinecraftOptionsStorage vanillaOpts) {
		OptionImpl<Options, ColorSpace> colorSpace = OptionImpl.createBuilder(ColorSpace.class, vanillaOpts)
			...
			.setBinding((options, value) -> {
					FirisVideoSettings.colorSpace = value;
					try {
						Firis.getFirisConfig().save();
					} catch (IOException e) {
						e.printStackTrace();
				...
				options -> FirisVideoSettings.colorSpace)
