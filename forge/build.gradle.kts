...
base {
    archivesName = "firis-forge-1.20.1"
}
...
mixin {
    add(sourceSets.main.get(), "firis.refmap.json")
    //add(project(":common").sourceSets.getByName("sodiumCompatibility"), "firis.refmap.json")
    setIgnoreConstraints(true)
    config("mixins.firis.json")
    config("mixins.firis.compat.dh.json")
    config("mixins.firis.compat.sodium.json")
    config("mixins.firis.vertexformat.json")
    config("firis-batched-entity-rendering.mixins.json")
    config("mixins.firis.fantastic.json")
    config("mixins.firis.forge.json")
}
...
val SODIUM_PATH = "fodium-0.6.0-alpha.2.jar"
...
dependencies {
    ...
    implementation(fg.deobf("net.caffeinemc:fodium:0.6.0"))
