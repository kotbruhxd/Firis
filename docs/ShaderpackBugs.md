...
This file tracks some bugs in shader packs that might appear to be Firis issues, but are just bugs within the shader pack.

## All shader packs

* Vanilla shadows appear in addition to dynamic shadows
    * Diagnosis: Shader packs force there to be shadows where there is low skylight to prevent light from leaking into caves. [While I would like to make this workaround unnecessary](https://github.com/FirisShaders/Firis/issues/317), it is entirely something implemented in the shaderpack, Firis doesn't really have control over it.
...
* Enchantment glints don't render
    ...
    * Confirmation: This bug has been confirmed to be observable in both Firis and OptiFine.
