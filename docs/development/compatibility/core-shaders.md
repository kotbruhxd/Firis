...
Executive Summary: Custom shaders added by mods or resource packs are ignored by Firis when an Firis shader pack is loaded. Mods and resource packs using custom shader behavior / vertex formats are extremely difficult to accomodate within Firis.

This issue thread also has useful information: https://github.com/FirisShaders/Firis/issues/1042

This document is *not* intended to inform the reader on how to make their custom shaders compatible with Firis shader packs. Rather, it's intended to provide an explanation for why the incompatibility exists. It's not because we don't care about compatibility - it's because this is a hard problem and there is no general solution to it.
...
## Shaders in Firis Shader Packs

Though Firis provides many features for shader packs, including support for shadow mapping, setting up certain inputs such as textures, uniforms, and extra vertex data, and ways to configure how the outputs of each shader program feed into other programs, most importantly it offers ways for shader packs to specify their own shader programs to be used for rendering content in Minecraft.
...
By providing a high degree of control over Minecraft's rendering, Firis allows shader packs to accomplish advanced graphical effects. However, this comes at the natural expense of compatibility with other mods or resource packs that attempt to modify rendering through their own shaders.

Though it is possible to fix many compatibility issues between shader packs and mods through improving the implementation of shader mods like Firis, some sources of compatibility issues fundamentally cannot be fixed. Custom shader programs are one example.

As a result, it is strongly advised that mods that wish to maintain compatibility with Firis shader packs avoid using their own shader programs if at all possible. If mods do use shaders for some rendering, for the purposes of compatibility with mods like Canvas and Firis, it is recommended to create fallback rendering pathways that do not use custom shaders.
